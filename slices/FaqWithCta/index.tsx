...
const FaqItem: FC<FaqItemProps> = ({ item, index, isOpen, onToggle }) => {
  return (
		<article className="py-5 sm:py-6 border-t border-slate-50">
			...
			>
				<span className="text-lg font-extrabold uppercase sm:text-xl">
			...
			>
				<div className="overflow-hidden">
					<div className="mt-3 max-w-2xl text-slate-200/90 sm:text-lg">
						<PrismicRichText
							field={item.answer}
							components={{
								paragraph: ({ children }) => (
		...
		<aside className="relative isolate overflow-hidden bg-slate-900/60 shrink-0 h-[600px] min-h-[600px]">
			...
			<div className="relative flex h-full flex-col justify-between gap-6 p-8">
				...
				<div className="space-y-3">
					<PrismicRichText
						field={primary.cta_title}
						components={{
							heading4: ({ children }) => (
								<h3 className="text-2xl font-black  sm:text-3xl">{children}</h3>
							),
							strong: ({ children }) => (
					...
					/>
					<div className=" text-slate-100/90 sm:text-lg">
						<PrismicRichText
							field={primary.cta_description}
							components={{
								paragraph: ({ children }) => (
									<p className="mb-3 last:mb-0">{children}</p>
								),
